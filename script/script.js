...
// ———————————————————————————————————————————————————————————————————————————————————————————————— LOAD COUPONS
const loadCoupons = () => {
    const savedCoupons = JSON.parse(localStorage.getItem("couponsAll")) || [];
    let currentPoints = parseInt(localStorage.getItem("currentPoints")) || 0;

    // Loop through the saved coupons and append them to their respective containers
    savedCoupons.forEach(coupon => {


        let couponPricePoints = parseInt(coupon.price)
    
         // Compare coupon price with currentPoints
        if (couponPricePoints < currentPoints) {
            // YES
            console.log("no");
            var disableCoupon = "";
        } else {
            ...
            var disableCoupon = "couponDisabled";
        }
        ...
        // Append to the appropriate container based on the "owned" property
        if (coupon.owned === "yes") {
            $(".availableCoupons").append(loadOwnedCoupon); // Owned coupons
        } else if (coupon.owned === "no") {
            $(".couponStoreList").append(loadStoreCoupon); // Available (not owned) coupons
        }
    });
};
